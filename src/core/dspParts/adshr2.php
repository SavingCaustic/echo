...
class ADSHR2 {
    //Timing in this adsr is logaritmic
    ...
    function setValues($a,$d,$s,$h,$r) {
        ...
        if ($d < 1) $d = 1; //what ? ms?
        if ($h < 1) $h = 1; //what ? ms?
        $this->aFactor = 1 - $this->srInv / $a; 
        echo 'A-factor: ' . $this->aFactor . "\r\n";
        $this->dFactor = $this->srInv / $d; 
        $this->sLevel = $s;
        $this->hFactor = $this->srInv / $h; 
        $this->rFactor = 1 - $this->srInv / $r; 
        echo 'R-factor: ' . $this->rFactor . "\r\n";
    }
    ...
    function getNextLevel() {
        switch ($this->state) {
            ...
            case 'SUSTAIN':
                ...
                break;
            case 'RELEASE':
                $this->level *= $this->rFactor;
                if ($this->level < 0.0001) $this->goIdle();
                break;
        }
        return $this->level;
    }
