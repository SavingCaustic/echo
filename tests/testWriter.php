...
class TestWriter {
    ...
    public function __construct($amp = 5000, $sampleRate = 48000, $isStereo) {
    ...
    function render($times) {
        for ($i = 0; $i < $times; $i++) {
            $this->playerEngine->renderNextBlock($i);
            $this->append($this->playerEngine->audioBuffer);
        }
    }

    private function writeHeader() {
        // Write WAV header
        $channels = ($this->isStereo ? 2 : 1);
        ...
        fwrite($this->fileHandle, pack('v', $channels)); // Number of channels
        fwrite($this->fileHandle, pack('V', $this->sampleRate)); // Sample rate
        fwrite($this->fileHandle, pack('V', $this->sampleRate * $channels * 2)); // Byte rate
        fwrite($this->fileHandle, pack('v', 2 * $channels)); // Block align
